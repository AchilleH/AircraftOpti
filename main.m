...
vtailh = 1.5*vtailc;
htailc = c;
Clwa = 0.1; %WING 2d lift coefficient and vs alpha [1/deg]
...
for i = 1:n
    ...
    %fuselageL = rand()*(10 - 5 - Lmot(j)) + 5 + Lmot(j);
    ...
    [CDi,CDo,CD,D,Ds,Di,Dis,Do,Tr,np,Pav,Tav,Pr] = DPT(length(aoarange),PE,CL,W,Swet,Swet,Aw,Sw,At,St,ew,et,T,c,tapw,CLw,CLt,xdivc,V);
        %% CG
